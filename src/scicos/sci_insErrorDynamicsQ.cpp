...
void sci_insErrorDynamicsQ(scicos_block *block, scicos::enumScicosFlags flag)
{
    ...
    if (flag==scicos::computeOutput)
    {
        ...
        static const double cL = cos(L);
        static const double sL = sin(L);

        static const int rowsF = 10 ;

        F[0+rowsF*0]=-a*d*wz-a*c*wy-a*b*wx;
        F[0+rowsF*1]=-((Omega*cL*sL-Omega*cL*cL+((3*d*d+(2*c+2*b)*d+c*c+b*b+a*a)*wz+(d*d+2*c*d-3*c*c+2*b*c+b*b+a*a)*wy+(d*d+2*b*d+(c*c)+2*b*c+3*(b*b)+(a*a))*wx)*cL)*R+Ve*sL+(Vn-Ve)*cL)/(2*cL*R);
        F[0+rowsF*4]=-c/(2*R);
	...
	F[0+rowsF*9]=(d*Ve*sL+(c*Vn-b*Ve)*cL)/(2*cL*(R*R));
        F[1+rowsF*0]=-((Omega*cL+(-4*b*d-6*a*c)*wz+(6*a*d-4*b*c)*wy+(3*(d*d)+3*(c*c)-(b*b)-3*(a*a))*wx)*R+Ve)/(2*R);
        F[1+rowsF*1]=((Omega*cL*sL+((-(d*d)+(4*a-2*c)*d-3*(c*c)-2*b*c-(b*b)+4*a*b+3*(a*a))*wz+(3*(d*d)+(6*c+2*b)*d+3*(c*c)+4*a*c+(b*b)+4*a*b-3*(a*a))*wy+(-6*a*d-6*a*c+2*a*b)*wx)*cL)*R+Ve*sL-Vn*cL)/(2*cL*R);
        F[1+rowsF*4]=-d/(2*R);
	...
	F[1+rowsF*9]=-(c*Ve*sL+(-d*Vn-a*Ve)*cL)/(2*cL*(R*R));
        F[2+rowsF*0]=(((4*c*d-6*a*b)*wz+(-3*(d*d)-(c*c)-3*(b*b)+3*(a*a))*wy+(6*a*d+4*b*c)*wx)*R+Vn)/(2*R);
        F[2+rowsF*1]=-((Omega*cL*sL+Omega*(cL*cL)+((-(d*d)+(-2*b-4*a)*d-(c*c)+(-2*b-4*a)*c-3*(b*b)+3*(a*a))*wz+(6*a*d+2*a*c+6*a*b)*wy+(3*(d*d)+(2*c+2*b)*d+(c*c)-4*a*c+(b*b)-4*a*b-3*(a*a))*wx)*cL)*R+Ve*sL+Ve*cL)/(2*cL*R);
        F[2+rowsF*4]=a/(2*R);
	...
	F[2+rowsF*7]=((d*Omega*(cL*cL)*sL-b*Omega*(cL*cL*cL))*R-b*Ve)/(2*(cL*cL)*R);
        ...
        F[3+rowsF*0]=((Omega*cL*sL+(((d*d)-3*(c*c)-3*(b*b)+3*(a*a))*wz+(4*c*d+6*a*b)*wy+(4*b*d-6*a*c)*wx)*cL)*R+Ve*sL)/(2*cL*R);
        F[3+rowsF*1]=((Omega*cL+(2*a*d-6*a*c-6*a*b)*wz+(-(d*d)+(4*a-2*b)*d-3*(c*c)+(4*a-6*b)*c-3*(b*b)+3*(a*a))*wy+((d*d)+(2*c+4*a)*d+3*(c*c)+2*b*c+(b*b)+4*a*b-3*(a*a))*wx)*R+Vn+Ve)/(2*R);
        F[3+rowsF*4]=b/(2*R);
        F[3+rowsF*5]=(a*sL+c*cL)/(2*cL*R);
	...
	F[6+rowsF*9]=((Vn*Vn)+(Ve*Ve))/(R*R);
        ...
        G[0+rowsG*1]=-(c*(d*d)*(R*R)-(c*c*c)*(R*R)+(b*b)*c*(R*R)+(a*a)*c*(R*R))/(2*(R*R));
        G[0+rowsG*2]=-((d*d*d)*(R*R)+(c*c)*d*(R*R)+(b*b)*d*(R*R)+(a*a)*d*(R*R))/(2*(R*R));
        G[1+rowsG*0]=(-3*a*(d*d)*(R*R)-3*a*(c*c)*(R*R)+a*(b*b)*(R*R)+(a*a*a)*(R*R))/(2*(R*R));
        G[1+rowsG*1]=((d*d*d)*(R*R)+3*(c*c)*d*(R*R)+(b*b)*d*(R*R)-3*(a*a)*d*(R*R)+4*a*b*c*(R*R))/(2*(R*R));
        G[1+rowsG*2]=(-c*(d*d)*(R*R)+4*a*b*d*(R*R)-(c*c*c)*(R*R)-(b*b)*c*(R*R)+3*(a*a)*c*(R*R))/(2*(R*R));
        G[2+rowsG*0]=-((d*d*d)*(R*R)+(c*c)*d*(R*R)+(b*b)*d*(R*R)-3*(a*a)*d*(R*R)-4*a*b*c*(R*R))/(2*(R*R));
        G[2+rowsG*1]=-(3*a*(d*d)*(R*R)+a*(c*c)*(R*R)+3*a*(b*b)*(R*R)-(a*a*a)*(R*R))/(2*(R*R));
        G[2+rowsG*2]=-(-b*(d*d)*(R*R)-4*a*c*d*(R*R)-b*(c*c)*(R*R)-(b*b*b)*(R*R)+3*(a*a)*b*(R*R))/(2*(R*R));
        G[3+rowsG*0]=(c*(d*d)*(R*R)+4*a*b*d*(R*R)+(c*c*c)*(R*R)+(b*b)*c*(R*R)-3*(a*a)*c*(R*R))/(2*(R*R));
        G[3+rowsG*1]=(-b*(d*d)*(R*R)+4*a*c*d*(R*R)-3*b*(c*c)*(R*R)-(b*b*b)*(R*R)+3*(a*a)*b*(R*R))/(2*(R*R));
        G[3+rowsG*2]=(a*(d*d)*(R*R)-3*a*(c*c)*(R*R)-3*a*(b*b)*(R*R)+(a*a*a)*(R*R))/(2*(R*R));
        G[4+rowsG*3]=-(d*d)-(c*c)+(b*b)+(a*a);
        G[4+rowsG*4]=2*(b*c-a*d);
        G[4+rowsG*5]=2*(b*d+a*c);
        G[5+rowsG*3]=2*(a*d+b*c);
        G[5+rowsG*4]=-(d*d)-(c*c)-(b*b)+(a*a);
        G[5+rowsG*5]=2*(c*d-a*b);
	G[6+rowsG*3]=2*(b*d-a*c);
	G[6+rowsG*4]=2*(c*d+a*b);
	G[6+rowsG*5]=(d*d)-(c*c)-(b*b)+(a*a);
    }
