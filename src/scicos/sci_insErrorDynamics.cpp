...
void sci_insErrorDynamics(scicos_block *block, scicos::enumScicosFlags flag)
{
    ...
    if (flag==scicos::computeOutput)
    {
        ...
        if (! (F&&G) )
        {
            Coserror((char *)"insErrorDynamics block initialization failed.");
        };
        ...
        // convert matrices to output vectors
        if (F) cMatrix2FortranVector(F,nX,nX,y1);
        if (G) cMatrix2FortranVector(G,nX,nU,y2);
    }
    ...
    {
        // don't allow block to reinitialize
        if(F || G) return;
        ...
        // note: allocateCMatrix initializes elements to zero
        allocateCMatrix(F,nX,nX);
        allocateCMatrix(G,nX,nU);
    }
    else
    {
