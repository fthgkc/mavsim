function [x,y,typ]=bodyDynamics(job,arg1,arg2)
//
// bodyDynamics.sci
//
// USAGE:
...
select job
	...
	case 'set' then
		...
		while %t do
			...
			elseif stateMode==2 then
				...
				error('invalid mode in bodyDynamics block');
			end
	...
	case 'define' then
		...
		model=scicos_model();
		model.sim=list('sci_bodyDynamics',4);
		...
		// setup icon
	  	gr_i=['xstringb(orig(1),orig(2),[''body dynamics''],sz(1),sz(2),''fill'');']
